...
namespace Game
{
    public class EnemyTank : GameObject, IShooter, IDamageable, IEnemy
    {
        //Campos
        ...
        //Constructor
        public EnemyTank(string name, int damage, float speed, Vector2 starPosition, Vector2 scale, float cooldown, Bullet bulletType, int maxLife, int maxAmmo, List<Animation> animations, float angle = 0, string tag = "Enemies") : base(name, starPosition, scale, angle)
        {
            Tag = tag;
            //Engine.Debug($"Se creo un EnemyTank de Tag: {this.Tag}");
            DisplayName = name;
            CurrentSpeed = speed;
            IsTrigger = true;
            moveLeft = true;
            ...
            ReloadMaxAmmo();
            CreateAnimations();
            currentAnimation = animationFactory.IdleAnimationEnemyTank;
        }
        ...
        private void Movement()
        {
            ...
            if (!moveLeft)
            {
                movementController.StartMovementX(this, CurrentSpeed, 0.5f);
        ...
        {
            animationFactory.CreateAnimationsEnemyTank();
        }
