...
namespace Game
{
    public class EnemyFast : GameObject, IShooter, IDamageable, IEnemy
    {
       //Campos
        ...
        //Constructor
        public EnemyFast(string name, int damage, float speed, Vector2 starPosition, Vector2 scale, float cooldown, Bullet bulletType, int maxLife, int maxAmmo, List<Animation> animations, float angle = 0, string tag = "Enemies") : base(name, starPosition, scale, angle)
        {
            Tag = tag;
            //Engine.Debug($"Se creo un EnemyFast de Tag: {this.Tag}");
            DisplayName = name;
            CurrentSpeed = speed;
            Cooldwon = cooldown;
            ...
            ReloadMaxAmmo();
            CreateAnimations();
            currentAnimation = animationFactory.IdleAnimationEnemyFast;
        }
        ...
        private void Movement()
        {
            ...
            if (this.Position.X > Program.WIDTH_SCREEN - 300)
            {
                moveLeft = true;
            }
        ...
        {
            animationFactory.CreateAnimationsEnemyFast();
        }
        ...
        public void Destroy()
        {
            ...
            IsDestroyed = true;
            OnDestroy?.Invoke(this);
            IItems items = ItemsFactory.CreateItem(Item.SpeedBoost, transform.Position);
        }
